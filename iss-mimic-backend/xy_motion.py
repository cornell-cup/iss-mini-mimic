...
# def bound_effort(value, max_effort=0.9):
#     return max(0, min(max_effort, value))


# class DefaultGantryParams:
#     MOTOR_REDUCTION = (9 / 24) * (9 / 27)  # in / out
#     PULLEY_TEETH = 36                      # teeth
#     PULLEY_PITCH = 2                       # mm
#     TURNS_TO_MM = MOTOR_REDUCTION * (PULLEY_TEETH * PULLEY_PITCH)
...
# class XYMotion:
#     @classmethod
#     def get_default_xy(cls, x_max=None, y_max=None):
#         motor_a = left_motor
#         motor_b = right_motor

#         motor_a.set_effort(0)
#         motor_b.set_effort(0)
...
#             x_max,
#             y_max,
#             DefaultGantryParams.TURNS_TO_MM,
#         )

#     def __init__(self, motor_a, motor_b,
#                  x_max=None, y_max=None,
#                  motor_turns_to_mm=DefaultGantryParams.TURNS_TO_MM):
...
#     def get_position(self):
#         a_motor_position = self.motor_a.get_position()  # rev
#         b_motor_position = self.motor_b.get_position()  # rev

#         a = (a_motor_position - self.zero_zero[0]) * self.turns_to_mm  # mm
#         b = (b_motor_position - self.zero_zero[1]) * self.turns_to_mm  # mm
...
#         print(f"Moving {x} in x and {y} in y. Traveling from {self.get_position()} to {(x_final, y_final)}")
#         print(f"Relative a: {relative_a_mm} mm and b: {relative_b_mm} mm")

#         await self.move_relative_ab(relative_a_turns,
#                                     relative_b_turns,
#                                     base_effort,
#                                     kp,
...
#         while True:
#             total_mm_diff = (
#                 abs(targetA - self.motor_a.get_position()) +
#                 abs(targetB - self.motor_b.get_position())
#             ) * self.turns_to_mm
...
#             delta_a_so_far = self.motor_a.get_position() - startingA
#             delta_a_to_done = targetA - self.motor_a.get_position()
#             a_fraction_done = 1 if delta_a_to_done == 0 or delta_a == 0 else delta_a_so_far / delta_a

#             delta_b_so_far = self.motor_b.get_position() - startingB
#             delta_b_to_done = targetB - self.motor_b.get_position()
#             b_fraction_done = 1 if delta_b_to_done == 0 or delta_b == 0 else delta_b_so_far / delta_b
...
#                    kp=0.2):
#         """
#         Bang the gantry into an endstop until both motors nearly stop moving.
#         Assumes motor_a and motor_b expose a .speed attribute (EncodedMotor).
#         """
...
#             self.motor_a.set_effort(
#                 A_Motor_Direction * bound_effort(base_effort - encoderCorrection)
#             )
#             self.motor_b.set_effort(
#                 B_Motor_Direction * bound_effort(base_effort + encoderCorrection)
#             )

#             try:
#                 if abs(self.motor_a.speed) <= 0.1 and abs(self.motor_b.speed) <= 0.1:
#                     self.stop()
#                     break
#             except AttributeError:
#                 if abs(deltaA) < 1e-3 and abs(deltaB) < 1e-3:
#                     self.stop()
#                     break
...
#         # base_effort: base effort for the motors
#         # kp: proportional gain for the motors
#         print(A_Motor_Direction)

#         # save initial encoder positions
#         startingA = self.motor_a.get_position()
#         startingB = self.motor_b.get_position()
#         print("startingA ", startingA)
...
# async def manual_control(gantry):
#     while True:
#         command = input("Enter command (home/find_size/move/position/exit): ").strip()
...
#             await gantry.move_to(x, y)
#         elif command == "position":
#             print("Current position (mm):", gantry.get_position())
#         else:
#             print("Invalid command")
